...
| P4.8 | **Worker korekcji GPS łapie trasy telefonu (batch), nie tylko live.** `tools/snap_interpolate.py`: `fetch_raw_points` przetwarza teraz WSZYSTKIE pending `source='raw'` (domyślnie `LOOKBACK_MINUTES=0` = drop okna 60 min; okno było tylko optymalizacją, bo przetworzone wiersze i tak stają się `snapped`). Dzięki temu trasa wgrywana **wsadowo po jeździe** (fix-czasy starsze niż 60 min — inaczej niż streamujące trackery) zostaje snapnięta/interpolowana w ≤5 min (timer systemd co 5 min). Dopełnienie: pojedyncze fixy (ride <2 pkt, nie do `/match`) też **wychodzą z raw** (`source='snapped'` z oryginalnymi współrzędnymi), więc nie ma wiecznego re-skanu. Wdrożone na malince (`/opt/mototracker/snap_interpolate.py` + `snap.env LOOKBACK_MINUTES=0`); zweryfikowane: `raw` spadło do 0. **App wysyła SUROWE punkty (`pathJson`), serwer koryguje** — spójnie ze sprzętowymi trackerami (raw = źródło prawdy, brak double-snap). | ✅ |
